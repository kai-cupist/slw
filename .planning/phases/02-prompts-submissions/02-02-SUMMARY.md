...
plan: 02
subsystem: api
tags: [nestjs, raw-sql, pagination, dto, rest-api, swagger]

requires:
  - phase: 02-prompts-submissions/01
    provides: "DB 스키마(prompts 테이블), DatabaseService, PaginationDto, PaginatedResponse, Envelope 패턴"
provides:
  ...
  - "Prompt 인터페이스"
affects: [02-prompts-submissions/03, submissions, evaluations]

tech-stack:
  added: []
  patterns:
    [
      "동적 WHERE 절 paramIndex 카운터 방식",
...
patterns-established:
  - "Repository 패턴: Raw SQL 쿼리를 Repository 레이어에 캡슐화"
  - "동적 필터링: paramIndex 카운터로 조건부 WHERE 절 + 파라미터 바인딩"
  - "COUNT/SELECT 동일 WHERE 공유: 페이지네이션 데이터 불일치 방지"
  - "ParseIntPipe: URL 파라미터 정수 변환"
