...
plan: 03
subsystem: api
tags: [nestjs, raw-sql, submissions, crud, pagination, soft-delete]

requires:
  - phase: 01-infra-setup
    provides: DatabaseService (query/queryOne/withTransaction), ResponseInterceptor, HttpExceptionFilter, ValidationPipe
  - phase: 02-prompts-submissions/01
    provides: DatabaseModule, MigrationModule, submissions 테이블 마이그레이션
  - phase: 02-prompts-submissions/02
    provides: PromptsModule (PromptsService exports), UserIdGuard, UserId 데코레이터, PaginationDto, PaginatedResponse
provides:
  ...
  - 소유권 검증 (userId 기반)
  - soft delete 패턴
affects: [03-ai-evaluation, frontend]
...
tech-stack:
  ...
  patterns:
    [
      상태 전환 규칙 서비스 레이어 구현,
      soft delete WHERE deleted_at IS NULL 패턴,
      동적 WHERE절 paramIndex 카운터,
      PromptsModule 의존성 주입으로 prompt_id 유효성 검증,
    ]
...
patterns-established:
  - "상태 전환 검증: service 레이어에서 status 체크 후 BadRequestException"
  - "소유권 검증: findOneByIdAndUser(id, userId)로 user_id 필터링"
  - "모듈 간 의존성: imports에 다른 모듈 추가하여 서비스 공유"
