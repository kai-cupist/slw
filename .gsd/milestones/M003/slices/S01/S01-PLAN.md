...
**Goal:** 탭 바에 expo-symbols 아이콘을 추가하고, Stack 헤더 뒤로가기 옵션을 명시적으로 정리하여 네비게이션 UI가 완성된 앱처럼 보이게 한다.
**Demo:** After this: 탭 바에 아이콘이 보이고, 주제 상세에서 뒤로가기가 깔끔하게 동작한다.

## Tasks
- [x] **T01: expo-symbols SymbolView로 탭 아이콘 추가, Stack 헤더 뒤로가기를 minimal 모드로 정비** — expo-symbols의 SymbolView를 사용해 탭 바에 아이콘을 추가하고, Stack.Screen의 헤더 뒤로가기 옵션을 명시적으로 설정한다.
...
7. `cd mobile && npx tsc --noEmit`을 실행해 타입 에러가 없음을 확인한다.
  - Estimate: 30m
  - Files: mobile/app/(tabs)/_layout.tsx, mobile/app/_layout.tsx
  - Verify: cd mobile && npx tsc --noEmit && grep -q 'tabBarIcon' app/\(tabs\)/_layout.tsx && grep -q 'headerBackButtonDisplayMode' app/_layout.tsx
