...
- **Global scope**: `/Users/cupist/workspace/slw/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/cupist/workspace/slw/.claude/notes/`)
...
2. Glob `/Users/cupist/workspace/slw/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/cupist/workspace/slw/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/cupist/workspace/slw/.claude/notes/` — works in any directory
